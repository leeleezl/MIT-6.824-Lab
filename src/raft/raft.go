...
type Raft struct {
	...
	//仅leader，经常改变的状态(成为leader时重置)
	nextIndex  []int //每个follower的log同步起点索引（初始化为leader log索引的最后一项）
	matchIndex []int //每个follower的log同步进度（初始化0），与nextIndex强关联

	//所有服务器选举相关状态
	role              string    //角色
	leaderId          int       //leader的编号
	lastActiveTime    time.Time //上次活跃时间（刷新时机：收到leader心跳、给其他candidates投票、请求其他节点投票）
	lastBroadcastTime time.Time //作为leader上次广播时间
...
type AppendEntriesArgs struct {
	Term         int         //领导人的任期号
	LeaderId     int         //领导人id，以便于跟随者重定向请求
